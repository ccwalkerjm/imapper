...
module.exports = function(connection, parsed, data, callback) {
	...
	"FLAGS": ["FLAGS", "INTERNALDATE", "RFC822.SIZE"]
    };

	  if (params[0].type === "ATOM" && macros.hasOwnProperty(params[0].value.toUpperCase())) {
    params = macros[params[0].value.toUpperCase()];
  }
    ...
    if (forceSeen && !flagsExist) {
      ...
      async.eachSeries(range,function (rangeMessage,cb) {
        ...
        ////end of fetch connection--fixed c.walker
        return callback();
			});
		});
